...
export function AskQuestionModule() {
  ...
  const handleSubmit = async () => {
    if (!gameId || !selectedTemplate || !selectedTargetTeamId) return;

    try {
      await askQuestion({
        gameId,
        questionId: selectedTemplate.question_id,
        body: {
            ...
            {selectedCategory && (
              <>
                ...
                <button
                  ...
                  className={`font-medium transition-colors ${!selectedTemplate ? 'text-indigo-600 font-bold' : 'hover:text-indigo-600'}`}
                >
              ...
              {selectedTemplate.placeholders &&
                Object.keys(selectedTemplate.placeholders).length > 0 && (
                  <div className="space-y-4 pt-4 border-t border-gray-100">
                    ...
                    {Object.keys(selectedTemplate.placeholders).map((key) => {
