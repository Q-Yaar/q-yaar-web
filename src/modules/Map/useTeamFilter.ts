import { Team, isPlayerTeam } from 'models/Team';
import { useState, useEffect, useMemo } from 'react';

export const useTeamFilter = (teamsData: Team[] | undefined) => {
  const STORAGE_KEY = 'map_selected_team_id';

  const playerTeams = useMemo(() => {
    return (teamsData || []).filter(isPlayerTeam);
  }, [teamsData]);

  const [selectedTeamFilter, setSelectedTeamFilter] = useState<string>(() => {
    return localStorage.getItem(STORAGE_KEY) || '';
  });

  // Handle validation and fallback
  useEffect(() => {
    if (!playerTeams || playerTeams.length === 0) return;

    const isValid = playerTeams.some((team) => team.team_id === selectedTeamFilter);

    if (!isValid) {
      const fallbackId = playerTeams[0].team_id;
      setSelectedTeamFilter(fallbackId);
      localStorage.setItem(STORAGE_KEY, fallbackId);
    }
  }, [playerTeams, selectedTeamFilter]);

  // Sync to localStorage on manual changes
  const handleSetSelectedTeam = (id: string) => {
    setSelectedTeamFilter(id);
    if (id) {
      localStorage.setItem(STORAGE_KEY, id);
    } else {
      localStorage.removeItem(STORAGE_KEY);
    }
  };

  return {
    selectedTeamFilter,
    setSelectedTeamFilter: handleSetSelectedTeam,
  };
};